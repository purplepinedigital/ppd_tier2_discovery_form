...
export default function Tier2Form() {
  ...
  const [responses, setResponses] = useState<(string | null)[]>(
    new Array(tier2Questions.length).fill(""),
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: tier2Error } = await supabase.from("tier2_temp").insert({
  ...
  const startIndex = formSections
    .slice(0, currentSection)
    .reduce((sum, section) => sum + section.questions.length, 0);
  const currentQuestions =
    currentFormSection?.questions.map((q) => ({
      prompt: q.prompt,
      guidance: q.guidance,
    })) || [];

  const progressPercentage = Math.round(
    (responses.filter((r) => r && r.trim()).length / tier2Questions.length) *
      100,
  );
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-gray-600">
                  {responses.filter((r) => r && r.trim()).length}/
                  {tier2Questions.length} answered
                </span>
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        handleResponseChange(globalIdx, e.target.value)
                      }
