...
export default function EngagementDetail() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "tier1" | "tier2" | "stages" | "deliverables" | "notes"
  >("overview");
  ...
  const handleDelete = async () => {
    if (
      !id ||
      !window.confirm(
        "Are you sure you want to delete this engagement? This cannot be undone.",
      )
    ) {
      return;
    }
  ...
  if (!engagement)
    return <div className="text-center py-8">Engagement not found</div>;

  const TabButton = ({
    name,
    label,
  }: {
    name: typeof activeTab;
    label: string;
  }) => (
    <button
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-600">
                    {engagement.client_email || "Not assigned"}
                  </p>
                </div>
                <div>
              ...
              [
                ...
                <>
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 font-bold text-purple-600">
                      {engagement.recommended_package?.toUpperCase() || "N/A"}
                    </p>
                  </div>
                ...
                <p className="text-gray-500">
                  No Tier 1 assessment submitted yet
                </p>
              )}
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              engagement.status === "completed" ? (
                <>
                  ...
                  <div className="border-t pt-4 mt-4">
                    <h4 className="font-semibold text-gray-900 mb-3">
                      Discovery Responses (31 Questions)
                    </h4>
                    <Tier2ResponsesDisplay
                      engagementId={engagement.id}
                      engagementTitle={engagement.title}
                    />
                  </div>
                ...
                <p className="text-gray-500">
                  No Tier 2 assessment submitted yet
                </p>
              )}
            </div>
          )}
...
function Tier1AssessmentDisplay({
  engagementId,
  engagementTitle,
}: {
  engagementId: string;
  ...
  if (!assessment)
    return <p className="text-gray-500">No assessment data found</p>;
            ...
            {typeof assessment.reasoning === "object" ? (
              <>
                {assessment.reasoning.scopeFit && (
                  <p>• Scope Fit: {assessment.reasoning.scopeFit}</p>
                )}
                {assessment.reasoning.budgetFit && (
                  <p>• Budget Fit: {assessment.reasoning.budgetFit}</p>
                )}
                {assessment.reasoning.timelineFit && (
                  <p>• Timeline Fit: {assessment.reasoning.timelineFit}</p>
                )}
                {assessment.reasoning.primaryFactor && (
                  <p>• Primary Factor: {assessment.reasoning.primaryFactor}</p>
                )}
              </>
            ) : (
...
function Tier2ResponsesDisplay({
  engagementId,
  engagementTitle,
}: {
  engagementId: string;
    ...
    <div className="space-y-4">
      <div className="mb-4 p-3 bg-blue-50 rounded">
        <p className="text-sm text-blue-800">
          <strong>
            {answeredCount} out of {tier2Questions.length}
          </strong>{" "}
          questions answered
        </p>
        <div className="w-full bg-blue-200 rounded-full h-2 mt-2 overflow-hidden">
          <div
            ...
            style={{
              width: `${(answeredCount / tier2Questions.length) * 100}%`,
            }}
          />
        </div>
      </div>
