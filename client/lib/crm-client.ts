...
export async function acceptInvitationAndCreateAccount(
  token: string,
  password: string,
  firstName: string,
  lastName: string,
...
export async function submitTier1Form(
  engagementId: string,
  formData: Tier1FormData,
) {
  try {
    const { data: user } = await supabase.auth.getUser();
    if (!user.user) {
...
export async function submitTier2Form(
  engagementId: string,
  formData: Tier2FormData,
) {
  try {
    const { data: user } = await supabase.auth.getUser();
    if (!user.user) {
...
export function isAwaitingTier1(engagement: any): boolean {
  return (
    engagement.status === "awaiting_tier1" && !engagement.tier1_submitted_at
  );
}

export function isAwaitingTier2(engagement: any): boolean {
  return (
    engagement.status === "tier1_submitted" && !engagement.tier2_submitted_at
  );
}

export function hasCompletedTier2(engagement: any): boolean {
  return (
    engagement.status !== "awaiting_tier1" && engagement.tier2_submitted_at
  );
}
...
export function isInStages(engagement: any): boolean {
  return (
    ...
    (engagement.tier2_submitted_at && !engagement.tier1_feedback)
  );
}
